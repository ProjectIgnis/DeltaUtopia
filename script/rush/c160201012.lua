...
function s.filter1(c,tp)
	return c:IsFaceup() and c:HasLevel() and Duel.IsExistingMatchingCard(s.filter2,tp,LOCATION_MZONE,0,1,c,c:GetLevel()) and not c:IsMaximumModeSide()
end
function s.filter2(c,lvl)
	return c:IsFaceup() and c:IsLevel(lvl) and not c:IsMaximumModeSide()
end
